...
$(function() {
	$("#cloneinpt").blur(function(){
		$("#layList").css("display","none");
	})
	$("#internet").mouseover(function(){
		layer.tips(curlang.WANtip, this);	
	})
	$(".device-img img").mouseover(function(){
		layer.tips(curlang.term, this);
	})
	$("#reset").mouseover(function(){
		layer.tips(curlang.reboottip, this);
	})
	$("#wifi").mouseover(function(){
		layer.tips(curlang.wifitip, this);
	})
	$("#Dbind").mouseover(function(){
		layer.tips(curlang.staticbind, this);
	})
	$("#Dlimit").mouseover(function(){
		layer.tips(curlang.qostip, this);
	})
	$("#getQos").mouseover(function(){
		layer.tips(curlang.qostooltip, this);
	})
	$('#wanImg').on('mouseover', function(){
		layer.tips(curlang.wantooltip, this,{tips: 1}); 
	});
...
function set_QOSchecked(val){
	...
	if(!(reg.test(val))){
        layer.msg(curlang.Enter_format, {icon: 5});
        return false;
	}
	return true;
}
...
//pppoeserver
	...
	function pppoecycle(cycletime){
        ...
        if(cycletime>65535||cycletime<0){
            layer.msg(curlang.scope_error, {icon: 5});
            return false;
        }
		if(!(cycle.test(cycletime))){
            layer.msg(curlang.address_error, {icon: 5});
            return false;
		}
		return true;
	}
	function Checkpppoeaddress(IP_val){
		...
		var reg = /[^0-9]/;
		if(IP_val==""){
			layer.msg(curlang.ser_null, {icon: 5});
			return false;
		}//IP为空
		if(CheckChinese(IP_val)){
			layer.msg(curlang.ser_Illicit, {icon: 5});
			return false;
		}//IP有中文字符
		if(IP_val.length < 7 || IP_val.length > 15){
			layer.msg(curlang.ser_error, {icon: 5});
			return false;
		}
		ip=IP_val.split(".");
		if(ip.length != 4){
		...
		if($("#pppoeRadio1").prop("checked")){
			...
			if($("#pppoesauth").val()==1){
                if(spap==0&&schap==0&&sms_chap==0&&sms_chapv2==0){
                    layer.msg(curlang.sauth_null,{icon:5});
                    return;
                }
			}
...
function pppoeaddcheck(){
    ...
    function pppoeaddset(ser_val) {
        ...
        if(!(addset.test(ser_val))){
            layer.msg(curlang.pppoe_Error, {icon: 5});
            return false;
        }
        return true;
    }
			...
			if(onlist==""||onlist==null||onlist==undefined){$("#p_onlist").html("");}else{
				for(var i=0;i<onlist.length;i++){
                    ...
                    onlistBuf+= '<div class="total-div"><span class="user_span user-bor2" style="width:5%">'+(i+1)+'</span>'+
                        		...
                        		'<span class="user_span user-bor2" style="width:11%">'+curlang.online+'</span>'+
								'<span class="user_span user-bor2" style="width:15%;border-right:1px solid #ccc;"><span class="p_setshow" data-id="0">'+curlang.edit+'</span><span class="del_pterm" data-id="0" style="margin-left:10px;">'+curlang.Remove+'</span></span></div>';
				}
			...
			if(oflist==""||oflist==null||oflist==undefined){$("#p_offlist").html("");}else{
				for(var i=0;i<oflist.length;i++){
                    ...
                    oflistBuf+= '<div class="total-div"><span class="user_span user-bor2" style="width:5%">'+(i+onlist.length+1)+'</span>'+
								...
								'<span class="user_span user-bor2" style="width:19%;"><input type="text" style="display:none;" class="p_inpt"  value="'+endtime+'"><span>'+endtime+'</span></span>'+
                        		'<span class="user_span user-bor2" style="width:11%">'+curlang.offline+'</span>'+
								'<span class="user_span user-bor2" style="width:15%;border-right:1px solid #ccc;"><span class="p_setshow" data-id="0">'+curlang.edit+'</span><span class="del_pterm" data-id="0" style="margin-left: 10px;">'+curlang.Remove+'</span></span></div>';
				}
...
function currentPage(pno,psize){
    ...
    var tempStr = '<span>'+curlang.total+num+curlang.recording+'</span><span>'+curlang.total+totalPage+curlang.pages+'</span><span>'+curlang.current+currentPage+curlang.pages+'</span>';
    if(currentPage>1){
        tempStr += '<a href="#" onClick="currentPage('+(1)+','+psize+')">'+curlang.home+'</a>';
        tempStr += '<a href="#" onClick="currentPage('+(currentPage-1)+','+psize+')">'+curlang.previous+'</a>';
    }else{
        tempStr += '<span>'+curlang.home+'</span>';
        tempStr += '<span>'+curlang.previous+'</span>';
    }
    if(currentPage<totalPage){
        tempStr += '<a href="#" onClick="currentPage('+(currentPage+1)+','+psize+')">'+curlang.next+'</a>';
        tempStr += '<a href="#" onClick="currentPage('+(totalPage)+','+psize+')">'+curlang.last+'</a>';
    }else{
        tempStr += '<span>'+curlang.next+'</span>';
        tempStr += '<span>'+curlang.last+'</span>';
    }
...
function onform_pppoeclient(obj){
    ...
    if(endtime==""){
        ...
        if(!(reg.test(endtime))){
            layer.msg(curlang.Expire_tips,{icon:5});
            return;
        }
    }
	$.ajax({
...
//路由器定时重启
function get_sysreboot_info(){
	$.ajax({
		type: 'get',
		url: '/goform/get_sysreboot_info',
		data:{},
		dataType:"json", 
		success: function(msg) {
			if((typeof msg)=="object"){var data=msg;}
			...
			if(data.enable==1){
				$("#timingReboot_on").prop("checked",true);
				...
				$("#timingReboot_off").prop("checked",true);
			...
			$("#timingRebootSel").val(data.time);
		},
		error:function(msg){}
	})
}
function set_sysreboot_info(){
	var enableNum;
	if($("#timingReboot_on").prop("checked")){
		enableNum=1
	}else{
		enableNum=0;
	}
	...
	$.ajax({
		type: 'post',
		url: '/goform/set_sysreboot_info',
		data:{"enable":enableNum,"time":$("#timingRebootSel").val()},
		dataType:"json", 
		success: function(msg) {
			showanimation(5);
		},
...
//获取wan pppoe
function get_wan_pppoe_info(){
	$.ajax({
	type:"get",
	url:"/goform/get_wan_pppoe_info",
	dataType:"json",
	success: function(msg){
		if((typeof msg)=="object"){var data=msg;}
	else{ var data=eval('('+msg+')');}
		$("#login_admin").val(data.pppoename);
		$("#pppoename_label").hide();
		$("#login_pass").val(data.pppoepsw);
		$("#pppoepass_label").hide();
		$("#pppoeServer").val(data.server_name);
		$("#pppoe_mtu").val(data.mtu);
	},
	error: function(msg){		
	}
})}
$("#pppoe_title").click(function(){
	get_wan_pppoe_info();
})
//获取wan信息
...
//获取wan口info
function getData_wan_info(data_val){
	$.ajax({
	type:"get",
	...
	if((data.connetype).toLowerCase()=='static'){
		$("#wan_connettype1").html(curlang.wanLinkMode);		
	...
	if(data_val.operationmode == "router")
	{
        if((data.connetype).toLowerCase()=='static'){
            $("#linkType").html(curlang.wanLinkMode);
        }else if((data.connetype).toLowerCase()=='dhcp'){
	...
	else if(data_val.operationmode == "WISP"){
		$("#linkType").html(curlang.WISPmode);
		showter=true;
	}else if(data_val.operationmode == "repeater"){
		$("#linkType").html(curlang.RelayMode);
		$(".panel_5").css("display","none");
        $(".menu a").eq(3).css("display","none");
		showter=false;
	}else if(data_val.operationmode == "client"){
		$("#linkType").html(curlang.ClientMode);
        $(".panel_5").css("display","none");
        $(".menu a").eq(3).css("display","none");
		showter=false;
	}else if(data_val.operationmode == "bridge"){
		$("#linkType").html(curlang.BridgeMode);
        $(".panel_5").css("display","none");
        $(".menu a").eq(3).css("display","none");
		showter=false;
	}
	$("#interIP").html(data.ip);
...
function getData_routerinfo(){
	...
	if(num.charAt(6)=="0")
	{$("#wanImg").attr("src","/admin/images/bus.png");}
	else{$("#wanImg").attr("src","/admin/images/busOK.png"); }
	if(num.charAt(5) == 1){$("#lan1").attr("src","/admin/images/busOK.png");}else{ $("#lan1").attr("src","/admin/images/bus.png");};
	if(num.charAt(4) == 1){ $("#lan2").attr("src","/admin/images/busOK.png");}else{ $("#lan2").attr("src","/admin/images/bus.png");};
	if(num.charAt(3)== 1){ $("#lan3").attr("src","/admin/images/busOK.png");}else{ $("#lan3").attr("src","/admin/images/bus.png");};
	if(num.charAt(2) == 1){ $("#lan4").attr("src","/admin/images/busOK.png");}else{ $("#lan4").attr("src","/admin/images/bus.png");};
	if(num.charAt(0) == 0){ 
	$("#linkStus").attr("src","/admin/images/linkOff.png");
	$("#lineStus").attr("src","/admin/images/wanOff.png");
	}else{ 	
	$("#linkStus").attr("src","/admin/images/linkOn.png");
	$("#lineStus").attr("src","/admin/images/wanOK.png");
	loginFlag();
	};
	if(num.charAt(1) == 0)
	{ 
...
function get_permission_state(){
	$.ajax({
		type: 'get',
		url: '/goform/get_superfunctionlist_info',
		data: {},
		dataType:"json",
		success: function(msg) {
			if((typeof msg)=="object"){var data=msg;}
			...
			if(data.laninfo==3){
				$("#more_lan").parent().show();
				$(".lancon").show();
				$("#lan_set").show();
			}else if(data.laninfo==2){
				$("#more_lan").parent().show();
				$("#lan_set").show();
				$("#lan_button").hide();
				$(".lancon").show();
			}else{
				$("#more_lan").parent().hide();
				$(".lancon").hide();
				$("#lan_set").hide();
			}
			if(data.waninfo==3){
				$("#more_internet").parent().show();
				$("#wan_speed_manage").show();
				$("#internet").show();
				$("#linkStus").show();
				$("#lineStus").show();
				$("#wanImg").show();
			}else if(data.waninfo==2){
				$("#more_internet").parent().show();
				$("#internet").show();
				$("#linkStus").show();
				$("#lineStus").show();
				$("#wanImg").show();
				$("#wan_pppoe_button").hide();
				$("#wan_dhcp_button").hide();
				$("#wan_static_button").hide();
			}else{
				$("#more_internet").parent().hide();
				$("#wan_speed_manage").hide();
				$("#internet").hide();
				$("#linkStus").hide();
				$("#lineStus").hide();
				$("#wanImg").hide();
			...
			if(data.wifiinfo==3){
				...
				$("#2_4G_wifi_set").show();
				$("#5G_wifi_set").show();
				$("#more_wifi").parent().show();
				$("#more_5Gwifi").parent().show();
			}else if(data.wifiinfo==2){
				$("#wifi").show();
				$("#2_4G_wifi_set").show();
				$("#5G_wifi_set").show();
				$("#more_wifi").parent().show();
				...
				$("#wifiSwtich_5G").hide()
				$("#wifi5Swtich").hide()
				$("#2_4G_wifiSwtich").hide()
				$("#wifi_2gssidStatus").hide();
				$("#wifi_5gssidStatus").hide();
				$("#wlan_buuton").hide();
				$("#5gwlan_button").hide();
			}else{
				$("#more_wifi").parent().hide();
				$("#more_5Gwifi").parent().hide();
				$("#2_4G_wifi_set").hide();
				$("#5G_wifi_set").hide();
				$("#wifi").hide();
			}
			if(data.dmzinfo==3){
				$(".webadmin").parent().show();
				$("#webadmin").show();
			}else if(data.dmzinfo==2){
				$(".webadmin").parent().show();
				$("#webadmin").show();
				$("#webAdminApply").hide();
			}else{
				$(".webadmin").parent().hide();
				$("#webadmin").hide();
			}
		},
		error:function(data){}
...
function get_permission(){
	...
	$.ajax({
		...
		success: function(msg) {
			...
			if(data.laninfo==3){
				$("#lanVisi").prop("checked",true);
				$("#lanEdit").prop("checked",true);
			}else if(data.laninfo==2){
				$("#lanVisi").prop("checked",true);
				$("#lanEdit").prop("checked",false);
			}else{
				$("#lanVisi").prop("checked",false);
				$("#lanEdit").prop("checked",false);
			}
			if(data.waninfo==3){
				$("#wanVisi").prop("checked",true);
				$("#wanEdit").prop("checked",true);
			}else if(data.waninfo==2){
				$("#wanVisi").prop("checked",true);
				$("#wanEdit").prop("checked",false);
			}else{
				$("#wanVisi").prop("checked",false);
				$("#wanEdit").prop("checked",false);
			}
			if(data.wifiinfo==3){
				$("#wifiVisi").prop("checked",true);
				$("#wifiEdit").prop("checked",true);
			}else if(data.wifiinfo==2){
				$("#wifiVisi").prop("checked",true);
				$("#wifiEdit").prop("checked",false);
			}else{
				$("#wifiVisi").prop("checked",false);
				$("#wifiEdit").prop("checked",false);
			}
			if(data.dmzinfo==3){
				$("#remoteVisi").prop("checked",true);
				$("#remoteEdit").prop("checked",true);
			}else if(data.dmzinfo==2){
				$("#remoteVisi").prop("checked",true);
				$("#remoteEdit").prop("checked",false);
			}else{
				$("#remoteVisi").prop("checked",false);
				$("#remoteEdit").prop("checked",false);
			}
		},
		error:function(data){}
...
function onform_permission(sta,laninfo,waninfo,wifiinfo,dmzinfo){
	showanimation(1);
	$.ajax({
	type: 'post',
	url: '/goform/set_superfunctionlist_info',
	data: {"flag":sta,"laninfo":laninfo,"waninfo":waninfo,"wifiinfo":wifiinfo,"dmzinfo":dmzinfo},
	dataType:"json",
	success: function(data) {
		if(data.result==0){
			showanimation(5);
...
function onform_setsuperadmin(){
	...
	$.ajax({
	type: 'post',
	url: '/goform/set_supermanage_info',
	data: {'user':'superadmin','routepwd':newpass},
	dataType:"json", 
	success: function(data) {
	//showMessig(data,0);
	//sleep(0);
    ...
    $.ajax({
        ...
        success:function (msg) {
			...
			for(var i=0;i<len;i++){
				...
				'<span class="term_span" style="width: 18%"><span class="term_edit" style="margin-right:10px;">'+curlang.edit+'</span><span class="term_del">'+curlang.Remove+'</span></span></div>';
			}
            ...
            $(".term_edit").click(function() {
                ...
                $("#set_Edittime").click(function(){
                    ...
                    set_limitTerm(Emac,starth1,endh1,startm1,endm1,starth2,endh2,startm2,endm2);
                })
            })
        ...
        success:function (msg) {
			...
			for(var i=0;i<len;i++){
				time_buff+='<div><span class="term_span" style="width: 10%;border-left:1px solid #ccc;">'+(i+1)+'</span>'+
                    '<span class="term_span" style="width: 41%;line-height: 19px"><p>'+data.list[i].name+'</p><p>'+data.list[i].mac+'</p></span>'+
                    '<span class="term_span" style="width: 29%;line-height: 19px"><p>'+data.list[i].time1+'</p><p>'+data.list[i].time2+'</p></span>'+
					'<span class="term_span" style="width: 20%"><span class="term_edit"><i class="icon iconfont">&#xe616;</i></span><span class="term_del">+</span></span></div>';
			}
			$("#termT_list").html(time_buff);
            $(".term_edit").click(function() {
...
//提交
function set_limitTerm(mac,starth1,endh1,startm1,endm1,starth2,endh2,startm2,endm2){
    if(!timingChenk(starth1,endh1,startm1,endm1,starth2,endh2,startm2,endm2)){
        return;
	}
    if(starth2==""&&startm2==""&&endh2==""&&endm2==""){
	...
	for(var j=0;j<i;j++)
	{
        ...
        if(termlist[j].link_type==0) {
            buf += '<div class="terminal-item">' +
                ...
                '<p class="p-margin">' + curlang.online_Time + '<span class="dev-time">' + termlist[j].link_time + '</span></p>' +
                '</div>' +
...
function check_led(){
    ...
    if(start_h==end_h&&start_m>end_m){
        layer.msg(curlang.ledsm_err,{icon: 5});
    	return;
	}
    return true;
}
...
function get_curTmie(){
    $.ajax({
        ...
        success: function(msg) {
            ...
            $("#current_time, #term_router_time").html(data.systemtime);
        },
        error:function(msg) {}
    })
}
...
function checkPPTP(){
	...
	var PT_MTU=CheckMTU($("#pptp_mtu").val());
...
function getData_Mpptp(){
	...
	function checkNASuser(){
		...
		if($("#NASuser_type").val() == "setftpuserpass"){
			...
			if(!user_pass($("#NASuser_user").val())){
				layer.msg(curlang.ftpsymb,{icon:5});
				return false;
			}
			...
			if(!user_pass($("#NASuserPassword").val())){
				layer.msg(curlang.sambasymb,{icon:5});
				return false;
			}
		}
		...
		$.ajax({
			...
			success: function(msg) {
				...
				for(var i=0;i<itemLength;i++){
					item="<li style='margin:0px;'><span class='sp-l' style='width:70px'>"+(itemLength-i)+"</span><span class='sp-l' id='Path"+i+"' style='width:488px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;text-align:left;padding-left:10px;'>"+data.list[i].path+"</span><span class='sp-l' style='width:120px;'><input id='FTP"+i+"' name='NAS' class='FTPbox' type='checkbox' /></span><span class='sp-r' style='width:137px;text-align:center;'><input type='checkbox' name='NAS' class='SAMBAbox' id='SAMBA"+i+"' /></span></li>";
	...
	function onform_Path(){
		...
		if(samb>16){
            layer.msg(curlang.samblen_err,{icon:5});
            return false;
		}
		$.ajax({
		type: 'post',
		...
		success: function(data) {
			...
			if(data.result==0){
                ...
                layer.msg(curlang.fspath_err, {icon: 5});
			}else if(data.result==3){
                layer.msg(curlang.fspath_U, {icon: 5});
			}
		},
	...
	function getData_FTPshare_info(){
		...
		success: function(msg) {
			...
			for(var i=0;i<data.list.length;i++){
				item="<li id='FtpPthLi"+(data.list.length-i)+"'><span class='sp-l' style='width:80px'>"+(data.list.length-i)+"</span><span class='sp-l' id='FtpPth"+(data.list.length-i)+"' style='width:610px;text-align:left;padding-left:10px;overflow:hidden;'>"+data.list[i].path+"</span><span class='sp-r FTPpth' id='rmFtp"+(data.list.length-i)+"' style='width:142px;text-align:center;color:red;cursor:pointer;'>"+curlang.Remove+"</span></li>";
	...
	function getData_SAMBAshare_info(){
		...
		success: function(msg) {
			...
			$($("#NASsamba_li_first").siblings()).remove();
			for(var i=0;i<data.list.length;i++){
				item="<li id='SamPthLi"+(data.list.length-i)+"' class='samb_num'><span class='sp-l' style='width:80px'>"+(data.list.length-i)+"</span><span class='sp-l' id='SAMBAPth"+(data.list.length-i)+"' style='width:600px;text-align:left;padding-left:10px;overflow:hidden;'>"+data.list[i].path+"</span><span class='sp-r SAMBApth' id='rmSAMBA"+(data.list.length-i)+"' style='width:135px;text-align:center;color:red;cursor:pointer;'>"+curlang.Remove+"</span></li>";
...
function getData_Forwarding_info(){
		...
		success: function(msg) {
			...
			for(var i=0;i<data.list.length;i++){
				item="<li style='margin:0px' id='ForwardingLi"+(data.list.length-i)+"'><input type='hidden' id='ForwardingProtocol"+(data.list.length-i)+"' value='"+data.list[i].protocol+"' /><span class='sp-l' style='width:60px'>"+(data.list.length-i)+"</span><span class='sp-l' id='ForwardingIP"+(data.list.length-i)+"' style='width:285px'>"+data.list[i].ip+"</span><span class='sp-l' id='ForwardingPort"+(data.list.length-i)+"' style='width:180px;'>"+data.list[i].port+"</span><span class='sp-l'  style='width:170px;text-align:center;'>"+data.list[i].protocol+"</span><span class='sp-r ForwardingBtn' id='rmForwarding"+(data.list.length-i)+"' style='width:120px;text-align:center;cursor:pointer;color:red;'>"+curlang.Remove+"</span></li>";
...
function checkForwarding(){
	...
	if(parseInt($("#ForwardingList_startport").val()) > parseInt($("#ForwardingList_endport").val())){
		layer.msg(curlang.porterror, {icon: 5}); 
		return false;
	}
	return true;
}
...
function getData_Vserver_info(){
		...
		success: function(msg) {
			...
			for(var i=0;i<data.list.length;i++){
				item="<li style='margin:0px;'><input type='hidden' id='vseverProtocol"+(data.list.length-i)+"' value='"+data.list[i].protocol+"'><span class='sp-l' style='width:60px'>"+(data.list.length-i)+"</span><span class='sp-l' id='vseverIp"+(data.list.length-i)+"' style='width:265px'>"+data.list[i].ip+"</span><span class='sp-l' id='vseverPu"+(data.list.length-i)+"' style='width:140px;'>"+data.list[i].pu_port+"</span><span class='sp-l' id='vseverPr"+(data.list.length-i)+"' style='width:140px'>"+data.list[i].pr_port+"</span><span class='sp-l'  style='width:115px;text-align:center;'>"+data.list[i].protocol+"</span><span class='sp-r VserverBtn' id='rmVserver"+(data.list.length-i)+"' style='cursor:pointer;width:90px;text-align:center;color:red;'>"+curlang.Remove+"</span></li>"
...
function showMorePage(){
	...
	if($.cookie("user")=="superadmin"){
		$("#permission").parent("li").css("display","block");
	}else{
		$("#permission").parent("li").css("display","none");
		get_permission_state();
	}
	$("#worldTime").parent("li").css("display","block");
        /**
        //更多 显示隐藏
...
function MshowMorePage(){
	...
	$.ajax({
		...
		success: function(msg){
			...
			if($.cookie("user")=="superadmin"){}else{
				get_permission_state();
			}
			$("#worldTime").css("display","block");
			// sleep(0);
		},
		error:function(msg){
			layer.msg(curlang.sysinfo);
